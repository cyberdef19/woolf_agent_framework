...
class StructuredOutputResultWorker(
    ...
    def _get_message(self, context: FinalResponseContext)->str:
        ...
        return f"""
                ...
                В результатах кроків містяться дані про джерела:
                -source_id - ідентифікатор документа джерела з локальної векторної бази;
                -source_url - url документа отриманого з мережі інтернет;
                Якщо ці ідентифікатори заповнені, то їх дані установлюй в аналогічні ідентифікатори фінальної відповіді:
