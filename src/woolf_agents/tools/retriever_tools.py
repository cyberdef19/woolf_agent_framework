...
async def retrieve_historical_sources(
    ...
    """ Пошук релевантних фрагментів історичних джерел
      ...
      Args:
            ...
            query: запит семантичного пошуку у векторній базі даних історичних джерел.
            top_k (int, optional): Максимальна кількість документів. Defaults to 5.
    
        Returns:
            list: Повертає список знайдених документів
    """    
...
async def get_adjacent_chunks(
    ...
    """Отримує сусідні фрагменти історичного джерела
        ...
        інтерпретації. 
     Args:
        source_id (str): ідентифікатор документу локальної бази; не url мережевого ресурсу
        chunk_index (int): ідентифікатор чанка з документу
        before (int): Кількість чанків перед цільовим чанком. Default: 1.
        after (int): Кількість чанків після цільовим чанком. Default: 1.
...
async def search_related_sources(
    ...
    """
        ...
        Цей інструмент для пошуку у локальній базі документів.
        Не використовується для пошуку у мережі Інтернет.
